...
<<<<<<< HEAD
		...
		for(int i = 0; i <MAX_RESTAURANT_SIZE; i++) {
			...
			k=1;
			while(Res[i] == null &&i+k <MAX_RESTAURANT_SIZE-1 && Res[i+k]==null) {
				k++;
			}
			if(i+k <MAX_RESTAURANT_SIZE-1&&Res[i+k] != null) {
				Res[i] = Res[i+k];
				Res[i+k] = null;
			}
...
<<<<<<< HEAD
		...
		for(int i = 0; i <MAX_REVIEW_SIZE; i++) {
			k=1;
			while(Rev[i] == null &&i+k <MAX_REVIEW_SIZE-1 && Rev[i+k]==null) {
				k++;
			}
			if(i+k <MAX_REVIEW_SIZE-1&&Rev[i+k] != null) {
				Rev[i] = Rev[i+k];
				Rev[i+k] = null;
			}
